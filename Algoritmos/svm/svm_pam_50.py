...
##Algoritmo de classificação utilizando Suport Vector Machine##
clf1 = svm.SVC(kernel='linear');

clf1.fit(x_train, y_train);

pred_amostra = clf1.predict(x_test);
...
d = {'SVM(Linear)': diferentes}

result = pd.DataFrame(data=d);
result = result.replace({'SVM(Linear)': rev});
